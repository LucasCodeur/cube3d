...
/*   Updated: 2026/01/26 11:39:17 by prigaudi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	handle_up_and_down(data, keycode);
...
int	release_move(int keycode, t_data *data)
{
	...
	if ((keycode == XK_w || keycode == XK_W) && data->keycode.up == true)
		data->keycode.up = false;
	if ((keycode == XK_d || keycode == XK_D)
		&& data->keycode.right == true)
		data->keycode.right = false;
	if ((keycode == XK_a || keycode == XK_A) && data->keycode.left == true)
		data->keycode.left = false;
	if (keycode == XK_Left && data->keycode.rotate_left == true)
		data->keycode.rotate_left = false;
	else if (keycode == XK_Right && data->keycode.rotate_right == true)
		data->keycode.rotate_right = false;
	if (keycode == XK_Escape && data->keycode.escape == true)
...
static bool	handle_up_and_down(t_data *data, int keycode)
{
	if (handle_up(data, keycode) == true)
		return (true);
	else if (handle_down(data, keycode) == true)
		return (true);
	return (false);
}
