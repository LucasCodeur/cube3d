...
/*   Updated: 2026/01/08 16:40:14 by prigaudi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Create sprite 8x8 with image 32x32 static t_img *get_wall(t_parsing *data)
{
	t_img	*wall32;
	...
	wall32->addr = mlx_get_parsing_addr(wall32->img, &wall32->bpp,
			&wall32->line_len, &wall32->endian);
	return (wall32);
}

static void	wall_to_map(t_parsing *data, t_img *wall, t_img *map, int x, int y)
{
	int				i;
	int				j;
	unsigned int	color;
...
// wall = 8 * 8px
int	display_map(t_parsing *data)
{
	t_img	*map;
	t_img	*wall;
	int		x;
	...
	map->img = mlx_new_image(data->mlx_ptr, (data->map->width) * 32,
			(data->map->height) * 32);
	map->addr = mlx_get_parsing_addr(map->img, &map->bpp, &map->line_len,
			&map->endian);
	y = 0;
